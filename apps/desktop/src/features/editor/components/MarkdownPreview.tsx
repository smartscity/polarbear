    ...
    () => {
      ...
      renderer.renderer.rules.fence = (tokens, index) => {
        ...
        return [
          ...
          `<figcaption><span>${renderer.utils.escapeHtml(language)}</span><button type="button" class="code-copy-button" data-code="${encodedCode}">${t("common.copy")}</button></figcaption>`,
...
function handlePreviewClick(event: MouseEvent<HTMLElement>, t: Translate) {
